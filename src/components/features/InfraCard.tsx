...
import { getRunAtStartup, setRunAtStartup, type StartupResult } from "../../lib/commands";
  ...
  return (
    <div className="rounded-lg border border-line bg-white/80 p-5 shadow-sm">
      <div className="mb-4 flex items-start justify-between gap-4">
        <div>
          <h2 className="text-base font-semibold text-ink">Core infrastructure</h2>
          <p className="mt-1 text-sm leading-6 text-neutral-500">
            Tray icon, hide-to-tray, single instance, and run-at-startup.
          </p>
        </div>
        <span className={`shrink-0 rounded-full px-2.5 py-1 text-xs font-medium ${statusStyles[startupStatus]}`}>
          {startupStatus}
        </span>
      </div>
      ...
      <div className="space-y-3">
        ...
        <div className="rounded-md border border-line p-3">
          ...
          {startupResult && (
            <p className="mt-2 text-xs text-neutral-500">{startupResult.detail}</p>
          )}
          {startupError && (
