...
function buildAccelerator(e: KeyboardEvent | React.KeyboardEvent): string | null {
  const parts: string[] = [];
 ...
 */
export function ShortcutInput({ value, onCommit, onClear, placeholder }: ShortcutInputProps) {
  ...
  const display = capturing ? draft || "Press a chord…" : value || placeholder || "Not set";
  ...
  return (
    <div
      ...
      onKeyUp={handleKeyUp}
      className={`flex min-h-[2.25rem] cursor-text items-center rounded-md border px-3 font-mono text-sm outline-none transition ${
        capturing
          ? "border-accent bg-accent/5 text-ink ring-2 ring-accent/30"
          : "border-line bg-white text-ink hover:border-neutral-300"
      }`}
    >
      <span className={value || capturing ? "" : "text-neutral-400"}>{display}</span>
    </div>
  );
}
