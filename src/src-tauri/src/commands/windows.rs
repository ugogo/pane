...
pub async fn prepare_capture_windows(app: AppHandle) -> Result<(), String> {
    ...
    if app.get_webview_window(AREA_SELECTOR_LABEL).is_none() {
        let area_started = Instant::now();
        create_area_selector_window(&app)?;
        eprintln!(
            "capture timing: prepared area selector in {}ms",
            area_started.elapsed().as_millis()
        );
    }
    ...
    if app.get_webview_window(CAPTURE_PREVIEW_LABEL).is_none() {
        ...
        eprintln!(
            "capture timing: prepared preview in {}ms",
            preview_started.elapsed().as_millis()
        );
    }

    eprintln!(
        "capture timing: prepared capture windows in {}ms",
...
pub async fn show_area_selector(app: AppHandle) -> Result<(), String> {
    ...
    eprintln!(
        "capture timing: area selector shown in {}ms",
        started.elapsed().as_millis()
    );
    Ok(())
}
...
pub async fn show_capture_preview(app: AppHandle, _width: u32, _height: u32) -> Result<(), String> {
    ...
    eprintln!(
        "capture timing: preview refresh requested in {}ms",
        started.elapsed().as_millis()
    );
    Ok(())
}
...
pub async fn preview_ready(window: WebviewWindow) -> Result<(), String> {
    ...
    eprintln!(
        "capture timing: preview ready shown in {}ms",
        started.elapsed().as_millis()
    );
    Ok(())
}
...
pub async fn commit_region_capture(
    ...
    let result = {
        ...
        eprintln!(
            "capture timing: region full monitor captured in {}ms",
    ...
    let (w, h) = (result.result.width, result.result.height);
    {
    ...
    eprintln!(
        "capture timing: region commit completed in {}ms",
        started.elapsed().as_millis()
    );
    Ok(())
}
