...
- Start from the shadcn baseline generated by `npx shadcn@latest init --template vite --base base --preset nova`.
- Add UI components with `npx shadcn@latest add <component>` and keep the Base UI backend. Do not add Radix UI.
- Use generated components from `apps/windows/src/components/ui/*` before creating local primitives.
- Keep `apps/windows/src/styles.css` close to the shadcn default theme. Do not reintroduce Pane-specific design tokens, premium shell styling, or app-wide custom palettes without explicit design direction.
- Use Tailwind utilities for layout and small adjustments around shadcn components.
- Optimize the main window for the default 800-900 px width. Avoid permanent left navigation unless there is enough width or a clear workflow need.
