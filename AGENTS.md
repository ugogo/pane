...
- **No `.js` config files**: all config is TypeScript (`eslint.config.ts`,
  `vite.config.ts`); the PostCSS pipeline lives in `vite.config.ts`.
...
- **React Compiler** is enabled (`babel-plugin-react-compiler` via the Vite
  React plugin), so components are auto-memoized — don't hand-add
